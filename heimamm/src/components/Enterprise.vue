  ...
  <div class="aoutBox">
    <div class="toplist">
      <a-form
        ...
        ref="refListform"
      >
        <a-form-item label="企业编号" name="eid">
          <a-input v-model:value="Listform.eid" placeholder="企业编号" />
        </a-form-item>
    ...
    >
      <a-form
        ...
        :wrapper-col="{ span: 18 }"
      >
        <a-form-item label="企业编号" required name="eid">
          <a-input v-model:value="addform.eid" size="large" />
        </a-form-item>
        <a-form-item label="企业名称" required name="name">
          <a-input v-model:value="addform.name" size="large" />
        </a-form-item>
        <a-form-item label="企业简称" required name="short_name">
          <a-input v-model:value="addform.short_name" size="large" />
        </a-form-item>
        <a-form-item label="企业简介" required name="intro">
          <a-input v-model:value="addform.intro" size="large" />
        </a-form-item>
        <a-form-item label="企业备注">
          <a-row>
    ...
    >
      <a-form
        ...
        :wrapper-col="{ span: 18 }"
      >
        <!-- <a-form-item label="企业编号" required name="rid">
...
export default {
  data() {
    return {
      ...
      tableColumns: [
        ...
        { title: "企业编号", dataIndex: "user_id", key: "user_id" },
  ...
  methods: {
    ...
    getUsers() {
      ...
      console.log(pramas);
      httpGet(`/enterprise/list`, pramas)
        .then((response) => {
    ...
    setModal1Visible() {
      this.$refs.refaddform
        ...
        .then(() => {
          ...
          httpPost(`/enterprise/add`, pramas)
            .then((response) => {
              console.log(response);
              let { code } = response;
              if (code == 200) {
    ...
    // 修改用户状态
    eidlStatus(id) {
      httpPost(`/enterprise/status`, { id: id })
        .then((response) => {
          console.log(response);
          let { code } = response;
          if (code == 200) {
    ...
    editok() {
      ...
      // console.log(this.newrecord);
      httpPost(`/enterprise/edit`, this.newrecord)
        .then((response) => {
          console.log(response);
          let { code } = response;
          if (code == 201) {
    ...
    removeUser(id) {
      ...
      Modal.confirm({
        ...
        cancelText: "取消",
        onOk() {
          httpPost(`/enterprise/remove`, { id: id })
            .then((response) => {
              console.log(response);
              let { code } = response;
              if (code == 200) {
